...
public:
    MOCK_METHOD(void, addOnInterfacesAddedCallback, (const OnInterfacesAddedCallback callback), (override));
    MOCK_METHOD(void, addOnInterfacesRemovedCallback, (const OnInterfacesRemovedCallback callback), (override));
    MOCK_METHOD(std::shared_ptr<IDBusDeviceProxy>, createDevice, (std::string objectPath), (override));
    MOCK_METHOD(std::shared_ptr<IDBusCharacteristicProxy>, createCharacteristic, (std::string objectPath), (override));
    MOCK_METHOD((std::map<sdbus::ObjectPath, InterfacesAndPropertiesMap>), managedObjects, (), (override));
};
...
TEST(DBusBluetoothManagerTests, shouldGetAndFindDevices) {
    ...
    EXPECT_CALL(*mockProxy, addOnInterfacesAddedCallback).WillOnce(::testing::SaveArg<0>(&addedCallback));
    EXPECT_CALL(*mockProxy, addOnInterfacesRemovedCallback).WillOnce(::testing::SaveArg<0>(&removedCallback));
        ...
        deviceInterface,
            {
                ...
                { sdbus::PropertyName{"UUIDs"},     sdbus::Variant{
